...
export class BoardingDetailsPage implements OnInit {
  ...
  this.loading = true;
  try {
    const b: any = await this.firebaseService.getDocument('boardings', id);
    if (b) {
      this.boarding = {
        id: b.id,
        name: b.name || 'Unknown',
      ...
      const t = await this.toastCtrl.create({
        message: 'Boarding not found or unavailable.',
    ...
    if (err.code === 'permission-denied') {
      const t = await this.toastCtrl.create({
        message: 'This boarding is not available to view.',
        duration: 2000, color: 'warning'
      });
      t.present();
    } else {
