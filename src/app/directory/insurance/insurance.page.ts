...
export class InsurancePage implements OnInit {
  ...
  async loadInsurances() {
    ...
    try {
      const res = await this.firebaseService.getFilteredInformation(
        'medical-insurance', 'status', '==', 'Active'
      );
      ...
      this.coverages = Array.from(
        new Set(this.insurances.map(i => i.coverage).filter(c => c && c !== 'N/A'))
      ).sort();

      this.cities = Array.from(
  new Set(
    this.insurances
      .map(i => i.city ?? '')                          // normalize to string
      .filter((c): c is string => !!c && c !== 'N/A')  // type guard to string
  )
).sort();
  ...
  applyFilters() {
    this.filteredInsurances = this.insurances.filter(ins => {
      const matchesSearch =
        ...
        ins.city?.toLowerCase().includes(this.searchTerm);
  ...
  // New: simple expand/collapse instead of opening a (deleted) details page
  toggleExpand(item: Insurance) {
