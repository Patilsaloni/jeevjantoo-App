...
export type FavType = 'clinic' | 'ngo' | 'event' | 'adoption' | 'abc' | 'other';
export interface FavoriteItem {
  id: string;
  type: FavType;
  name: string;
...
export class FavoritesPage implements OnInit, OnDestroy {
  ...
  async loadFavorites() {
    try {
      const favPets: any[] = await this.firebase.getFavoritePets('pet-adoption');
      const favAbcs: any[] = await this.firebase.getFavoritePets('abc');

      this.items = [
        ...(favPets || []).map((p: any): FavoriteItem => {
          const typeRaw = (p.type || p.category || p.species || 'adoption').toString().toLowerCase();
          const type: FavType = (['clinic','ngo','event','adoption'].includes(typeRaw) ? typeRaw : 'adoption') as FavType;
          const city = p.city || this.extractCity(p.location) || '';
          return {
        ...
        ...(favAbcs || []).map((a: any): FavoriteItem => {
          const city = a.city || this.extractCity(a.location) || '';
          return {
            id: a.id,
            type: 'abc',
            name: a.name || a.type || 'Untitled',
            image: a.image || 'assets/default-thumb.png',
            city,
            area: a.area || '',
            address: a.address || a.location || '',
            lat: this.num(a.lat),
            lng: this.num(a.lng),
            contact: a.contact || a.contactNumber || '',
            status: a.status as any,
            tags: Array.isArray(a.tags) ? a.tags : [],
            dateAdded: typeof a.dateAdded === 'number' ? a.dateAdded : Date.now(),
            _raw: a,
          };
        }),
      ];
  ...
  applyFilters() {
    const q = this.searchQuery.trim().toLowerCase();
    const isDir = (t: FavType) => ['clinic','ngo','event','abc','other'].includes(t);
  ...
  openDetails(item: FavoriteItem) {
    const route = item.type === 'abc' ? `/tabs/directory/abc/${item.id}` : `/tabs/adoption/${item.id}`;
  ...
  async removeFavorite(item: FavoriteItem) {
    ...
    this.applyFilters();
    try {
      const namespace = item.type === 'abc' ? 'abc' : 'pet-adoption';
      await this.firebase.setFavorite(namespace, item.id, false);
      this.toastQuick('Removed from favorites');
    } catch (e) {
      console.error(e);
      this.items = prev;
